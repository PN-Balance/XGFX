...
static uint8_t * prv_GFX_Align_Color_Ptr( uint8_t * Ptr )
{
    /* ARMCC 5 is configured in C99 mode in this hardware project.  For the
       selected RGB565 color type, sizeof(GFX_Color_t) is also its required
       alignment and avoids the C11-only _Alignof operator. */
